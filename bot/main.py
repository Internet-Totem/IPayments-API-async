...
bot = Bot(token=TOKEN_TELEGRAM, parse_mode='Markdown')
dp = Dispatcher(bot)
...
async def start(message):
    if len(message.text.split('_')) == 1: # Стандартная комманда
        ...
        await bot.send_message(message.chat.id, '*Приветствую в Enot Shop!*', reply_markup=reply)
        ...
        if 'newPay' in msg_type[1]: # Сообщение о новой оплате
            ...
            if invoice['result']['status'] == 'PAID':
                ...
                if invoice['result']['amount'] == 5500:
                    await bot.send_message(message.chat.id, '*К сожалению, сейчас не удалось найти машину времени в наличии.*\n\nВся сумма была возвращена.')
                    await api_help.api(TOKEN_IPAYMENTS)
                    await api.refundInvoice(invoice_id) # Отмена оплаченного счета

                elif invoice['result']['amount'] == 14200:
                    userEmail = invoice['result']['email']
                    await bot.send_message(message.chat.id, '*Билеты были отправлены на почту:* ' + userEmail)

                elif invoice['result']['amount'] == 245.3:
                    await bot.send_message(message.chat.id, '*Покупка оплачена успешно!*\n\nВсе оплаченные товары: карандаши, тетради, ручка и блокнот.')

            else:
                await bot.send_message(message.chat.id, '*Не удалось найти оплату!*\nВозможно, кто-то пролил кофе на сервер.')
...
async def text(message):
    ...
    await bot.send_message(message.chat.id, 'Выберите товар который хотите купить.', reply_markup=reply)
    ...
    if call.message:
        ...
        if call_type == 'buy': # Купить товар
            ...
            if tovar == 'TimeMachine':
                ...
                api = await api_help.api(TOKEN_IPAYMENTS)
                invoice = await api.createInvoice(amount, product_list, name=name, description=description, success_btn_bot=myBot, success_text=success_text)

                reply = types.InlineKeyboardMarkup()
                reply.add(types.InlineKeyboardButton(text='Оплатить', url=invoice['result']['url']))

                await bot.edit_message_text('Счет на оплату готов.\n\n*Машина времени\nКол-во:* 1 шт.\n*Цена:* 5500 RUB\n\nОплатите счет в течении 20 минут. \
*После оплаты перейдите по выданной кнопке.*',
                ...
                api = await api_help.api(TOKEN_IPAYMENTS)
                invoice = await api.createInvoice(amount, product_list, success_btn, name=name, success_btn_url=myURL, success_text=success_text, need_email=need_email)
...
*После оплаты перейдите по выданной кнопке.*',
                ...
                api = await api_help.api(TOKEN_IPAYMENTS)
                invoice = await api.createInvoice(amount, product_list, description=description, success_btn_bot=myBot)
